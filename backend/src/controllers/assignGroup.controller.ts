import {
  ...
  getAppAssignedGroups,
} from '../services/assignGroup.service';
import { Request, Response, NextFunction } from 'express';
import mongoose from 'mongoose';

export const assignToGroups = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  try {
    ...
    res
      ...
      .json({ error: 'Assignment failed', details: err.message || 'Server Error' });
  }
};

export const unassignFromGroup = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  try {
    ...
    res
      .status((err as any).status || 500)
      .json({ error: 'Unassignment failed', details: err.message || 'Server Error' });
  }
};
...
export const getAssignedGroups = async (
  ...
  try {
    ...
    res
      .status((err as any).status || 500)
      .json({ error: 'Could not fetch group assignments', details: err.message || 'Server Error' });
  }
};
