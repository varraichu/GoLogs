...
export async function getAppsHealthData(userId: mongoose.Types.ObjectId) {
    ...
    const isAdmin = adminGroup && await UserGroupMember.findOne({ user_id: userId, group_id: adminGroup._id, is_active: true })

    let apps
    if (isAdmin) {
        // Admins can access all active applications
        apps = await Application.find({ is_deleted: false, is_active: true }).select('_id name created_at')
    } else {
        // Non-admins access apps through their user groups
        const groupIds = await UserGroupMember.find({ user_id: userId, is_active: true }).distinct('group_id')
        const appIds = await UserGroupApplication.find({ group_id: { $in: groupIds }, is_active: true }).distinct('app_id')
        apps = await Application.find({ _id: { $in: appIds }, is_deleted: false, is_active: true }).select('_id name created_at')
    }
    ...
    const criticalApps: CriticalApp[] = await Logs.aggregate([
        ...
        },
        {
            $project: {
                ...
                errors: {
                    $reduce: {
                        input: '$counts', initialValue: 0,
                        in: { $cond: [{ $eq: ['$$this.type', 'error'] }, { $add: ['$$value', '$$this.count'] }, '$$value'] }
                    }
                },
                warnings: {
                    $reduce: {
                        input: '$counts', initialValue: 0,
                        in: { $cond: [{ $eq: ['$$this.type', 'warn'] }, { $add: ['$$value', '$$this.count'] }, '$$value'] }
                    }
                }
            }
        },
